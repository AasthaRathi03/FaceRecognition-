from tkinter import*
from tkinter import ttk
from PIL import Image,ImageTk
from student import Student
from tkinter import messagebox
import os
from train import Train
from face_recognition import Face_Recognition
from attendance import Attendance
from datetime import datetime
import csv


class Face_Recognition_System:
    def __init__(self,root):
        self.root=root
        self.root.geometry("1366x768+0+0")
        self.root.title("Face Recognition System")
        # first image
        img = Image.open(r"C:\Users\Aastha Rathi\Downloads\face.jpg")
        img = img.resize((455,180), Image.LANCZOS)
        self.photoimg = ImageTk.PhotoImage(img)

        f_lbl=Label(self.root,image=self.photoimg)
        f_lbl.place(x=0,y=0,width=455,height=180)

        #second image

        img1= Image.open(r"C:\Users\Aastha Rathi\Downloads\main.jpg")
        img1= img1.resize((455,180), Image.LANCZOS)
        self.photoimg1 = ImageTk.PhotoImage(img1)

        f_lbl=Label(self.root,image=self.photoimg1)
        f_lbl.place(x=455,y=0,width=455,height=180)

        #third image


        img2 = Image.open(r"C:\Users\Aastha Rathi\Downloads\next.jpg")
        img2= img2.resize((456,180), Image.LANCZOS)
        self.photoimg2 = ImageTk.PhotoImage(img2)

        f_lbl=Label(self.root,image=self.photoimg2)
        f_lbl.place(x=910,y=0,width=456,height=180)








        #bg image

        img3 = Image.open(r"C:\Users\Aastha Rathi\Downloads\bg.jpg")
        img3 = img3.resize((1366,588), Image.LANCZOS)
        self.photoimg3 = ImageTk.PhotoImage(img3)

        bg_img=Label(self.root,image=self.photoimg3)
        bg_img.place(x=0,y=180,width=1366,height=588)

        title_lbl = Label(bg_img,text="FACE RECOGNITION ATTENDANCE SYSTEM SOFTWARE",font=("times new roman", 35,"bold"),bg="white",fg="red")
        title_lbl.place(x=0,y=0,width=1366,height=45)


        #student button

        img4 = Image.open(r"C:\Users\Aastha Rathi\Downloads\students.jpg")
        img4 = img4.resize((220,220), Image.LANCZOS)
        self.photoimg4 = ImageTk.PhotoImage(img4)


        b1=Button(bg_img,image=self.photoimg4,command =self.student_details,cursor="hand2")
        b1.place(x=200,y=60,width=220,height=220)

        b1_1=Button(bg_img,text="Student Details",command =self.student_details,cursor="hand2",font=("times new roman",15 ,"bold"),bg="dark blue",fg="white")
        b1_1.place(x=200,y=260,width=220,height=40)

        #detect face button

        img5 = Image.open(r"C:\Users\Aastha Rathi\Downloads\detector.jpg")
        img5 = img5.resize((220,220), Image.LANCZOS)
        self.photoimg5 = ImageTk.PhotoImage(img5)


        b1=Button(bg_img,image=self.photoimg5,cursor="hand2",command=self.face_detector)
        b1.place(x=460,y=60,width=220,height=220)

        b1_1=Button(bg_img,text="Face Detector",cursor="hand2",command=self.face_detector,font=("times new roman",15 ,"bold"),bg="dark blue",fg="white")
        b1_1.place(x=460,y=260,width=220,height=40)


        #attendance face button

        img6 = Image.open(r"C:\Users\Aastha Rathi\Downloads\attendance.jpg")
        img6 = img6.resize((220,220), Image.LANCZOS)
        self.photoimg6 = ImageTk.PhotoImage(img6)


        b1=Button(bg_img,image=self.photoimg6,cursor="hand2",command=self.attendance_data)
        b1.place(x=720,y=60,width=220,height=220)

        b1_1=Button(bg_img,text="Attendance",cursor="hand2",command=self.attendance_data,font=("times new roman",15 ,"bold"),bg="dark blue",fg="white")
        b1_1.place(x=720,y=260,width=220,height=40)

        #Help desk button 

        img7 = Image.open(r"C:\Users\Aastha Rathi\Downloads\help.jpg")
        img7 = img7.resize((220,220), Image.LANCZOS)
        self.photoimg7 = ImageTk.PhotoImage(img7)


        b1=Button(bg_img,image=self.photoimg7,cursor="hand2",command=self.help_data)
        b1.place(x=980,y=60,width=220,height=220)

        b1_1=Button(bg_img,text="Help Desk",cursor="hand2",command=self.help_data,
            font=("times new roman",15 ,"bold"),
            bg="dark blue",fg="white")
        b1_1.place(x=980,y=260,width=220,height=40)


       #Train face button 

        img8 = Image.open(r"C:\Users\Aastha Rathi\Downloads\Modeltrain.jpg")
        img8 = img8.resize((220,220), Image.LANCZOS)
        self.photoimg8 = ImageTk.PhotoImage(img8)


        b1=Button(bg_img,image=self.photoimg8,cursor="hand2",command=self.train_data)
        b1.place(x=200,y=300,width=220,height=220)

        b1_1=Button(bg_img,text="Train data",cursor="hand2",command=self.train_data,font=("times new roman",15 ,"bold"),bg="dark blue",fg="white")
        b1_1.place(x=200,y=500,width=220,height=40)

        #photos button


        img9 = Image.open(r"C:\Users\Aastha Rathi\Downloads\photos.jpg")
        img9 = img9.resize((220,220), Image.LANCZOS)
        self.photoimg9 = ImageTk.PhotoImage(img9)


        b1=Button(bg_img,image=self.photoimg9,cursor="hand2",command=self.open_img)
        b1.place(x=460,y=300,width=220,height=220)

        b1_1=Button(bg_img,text="Photos",cursor="hand2",command=self.open_img,font=("times new roman",15 ,"bold"),bg="dark blue",fg="white")
        b1_1.place(x=460,y=500,width=220,height=40)

        #Developer button
        
        
        img10 = Image.open(r"C:\Users\Aastha Rathi\Downloads\developer.jpg")
        img10 = img10.resize((220,220), Image.LANCZOS)
        self.photoimg10 = ImageTk.PhotoImage(img10)


        b1=Button(bg_img,image=self.photoimg10,cursor="hand2",command=self.attendance_data)
        b1.place(x=720,y=300,width=220,height=220)

        b1_1=Button(bg_img,text="Attendance Report",cursor="hand2",
            command=self.attendance_data,
            font=("times new roman",15 ,"bold"),
            bg="dark blue",fg="white")
        b1_1.place(x=720,y=500,width=220,height=40)


        #exit button

        img11 = Image.open(r"C:\Users\Aastha Rathi\Downloads\exit.jpg")
        img11 = img11.resize((220,220), Image.LANCZOS)
        self.photoimg11 = ImageTk.PhotoImage(img11)


        b1=Button(bg_img,image=self.photoimg11,cursor="hand2",command=self.iExit)
        b1.place(x=980,y=300,width=220,height=220)

        b1_1=Button(bg_img,text="Exit",cursor="hand2",command=self.iExit,
            font=("times new roman",15 ,"bold"),
            bg="red",fg="white")
        b1_1.place(x=980,y=500,width=220,height=40)

        #==========functions button =============

    def student_details(self):
            self.new_window=Toplevel(self.root)
            self.app=Student(self.new_window)
            
    def train_data(self):
            self.new_window=Toplevel(self.root)
            self.app=Train(self.new_window)
            
    def open_img(self):
        if os.path.exists("data"):
            os.startfile("data")
        else:
            messagebox.showerror("Error", "Data folder not found")
            
    def face_detector(self):
        self.new_window = Toplevel(self.root)
        self.app = Face_Recognition(self.new_window)
        
    def attendance_data(self):
        
        if hasattr(self, 'attendance_window') and self.attendance_window.winfo_exists():
            self.attendance_window.focus()
        else:
            self.new_window = Toplevel(self.root)
            self.app = Attendance(self.new_window)
        
        
    def iExit(self):
        iExit = messagebox.askyesno("Face Recognition", "Are you sure you want to exit?")
        if iExit:
            self.root.destroy()
            
            
    def help_data(self):
        messagebox.showinfo(
        "Help Desk",
        "Face Recognition Attendance System\n\n"
        "1. Add Student Details\n"
        "2. Capture Face Samples\n"
        "3. Train Data\n"
        "4. Click Face Detector\n"
        "5. Attendance will be marked automatically\n"
        "6. View Attendance Report\n\n"
        "Developed for MCA Project"
    )

    
            



if __name__ == "__main__":
    root=Tk()
    obj=Face_Recognition_System(root)
    root.mainloop()
